...
CREATE TABLE IF NOT EXISTS `pulso-madrid.facts.cercanias_observed_trains` (
  ...
  schedule_relationship STRING             OPTIONS(description="'SCHEDULED' or 'CANCELED', as published. A cancelled trip carries no stopTimeUpdate and no vehicle entity, which is how a cancellation is distinguished from a train that merely stopped being reported. NOTE: the reverse does not hold — a row with no vehicle columns is NOT necessarily cancelled. Over 148 publications in one hour, 337 rows were SCHEDULED with no vehicle entity, and they came from exactly 3 publications in which every Madrid train lacked one: Renfe serving a partial vehicle_positions feed while trip_updates was complete. 145 publications had every train tracked and none was partly tracked, so this failure is all-or-nothing per publication. NULL only where the trip update fetch failed for this publication"),
